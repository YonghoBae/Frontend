...
//동적으로 서버에서 만들어진 HTML소스를 웹브라우저로 가져와 해당 영역에 표시
//SEO(검색엔진 최적화시 주로 사용하거나 또는 CSR로딩 지연이 발생시 SSR로 대체가능)

import { Article } from "@/interface/article";

const SSR = ({articles}:{articles:Article[]}) => {
...
//getServerSideProps()함수는 해당 컴포넌트를 최초로 화면에 렌더링시에 자동으로 먼저 호출
//SSR실행순서: getServerSideProps()함수 실행 및 결과값 pros전달(서버에서 실행) -> 해당컴포넌트 실행(props)-서버에서 실행->return 구문실행(jsx)서버에 최종 결과화면 
//웹브라우저 표시
